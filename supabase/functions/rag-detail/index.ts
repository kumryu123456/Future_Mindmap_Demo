...
async function enrichNodeContent(
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'gpt-4', // Use GPT-4 for better content enrichment
      messages: [
        {
          role: 'system',
...
async function cacheEnrichedContent(
  ...
  const cacheData = {
    ...
    content_id: nodeId,
    content_text: `${enrichedContent.detailed_description} ${enrichedContent.key_concepts?.join(', ') || ''}`,
    metadata: {
    ...
    if (method === 'POST') {
      ...
      if (cachedEnrichment) {
        ...
        return new Response(
          JSON.stringify({ 
            data: {
              ...cachedEnrichment.enriched_content,
              relevant_sources: cachedEnrichment.relevant_sources || [],
              enrichment_context: cachedEnrichment.enrichment_context || {},
              cached: true,
