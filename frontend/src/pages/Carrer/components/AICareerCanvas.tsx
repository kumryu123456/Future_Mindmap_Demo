...
import { type CareerNode as CareerNodeType } from "@/types/career";
import {
  createSmartMindmapSimple,
  convertToXYFlowNodes,
  convertToXYFlowEdges,
} from "@/services/smartMindmapApi";
import { quickExpandNode, expandMemoryNode } from "@/services/nodeExpandApi";
import { PlusIcon } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useTheme } from "@/components/theme-provider";
import "@/pages/Carrer/components/AICareerCanvas.css";
...
const CenterNode = ({
  ...
  return (
    <div
      className={`career-node center-node ${theme} ${selected ? "selected" : ""}`}
    >
      {/* 연결 핸들 (클릭으로 노드 생성) */}
  ...
  const handleExpandNode = async (
    ...
    try {
      ...
      if (!result.success && result.error?.code === "404") {
        console.log(
          "Database node not found, trying memory expansion for button click...",
        );
      ...
      if (result.success && result.nodes && result.edges) {
        ...
        toast.show(`${result.nodes.length}개의 하위 노드가 생성되었습니다!`, {
          type: "success",
        });
      } else {
      ...
      toast.show("노드 확장 중 오류가 발생했습니다.", { type: "error" });
    } finally {
...
const DetailNode = ({ data }: { data: any; id: string }) => {
  const { theme } = useTheme();
...
const ExpandedNode = ({ data }: { data: any; id: string }) => {
  const { theme } = useTheme();

  return (
    <div
      className={`career-node expanded-node ${theme}`}
...
const MinorNode = ({ data }: { data: any; id: string }) => {
  const { theme } = useTheme();
...
const convertCareerMapToFlow = (
  careerMap: any,
  calculateOptimalConnection: Function,
    ...
    careerMap.connections.forEach((connection: any) => {
      ...
      if (sourceNode && targetNode) {
        ...
        console.log("✅ 원본 연결 처리:", {
          ...
          handles: {
            source: optimal.sourceHandle,
            target: optimal.targetHandle,
          },
        });
      }
    });
...
const Sidebar: React.FC<{
  ...
  const careerInfo = {
    title: "AI 개발자",
    description:
      "인공지능 기술을 활용하여 어플리케이션과 시스템을 개발하는 전문가",
  ...
  const reviews = [
    {
      ...
      date: "2024-01-15",
      content:
        "전공 지식이 없어도 충분히 도전할 수 있는 분야인 것 같아요. 저도 비전공자로서 성공적으로 전환했습니다.",
      isRead: false,
    },
    {
      id: "2",
      ...
      date: "2024-01-14",
      content:
        "사업 구조 및 기술 스택에 대한 자세한 설명이 도움이 되었습니다. 감사합니다!",
      isRead: false,
    },
    {
      id: "3",
      ...
      date: "2024-01-13",
      content:
        "PM으로 전환하려는 분들께 강추합니다. 실무 경험 강화 방법이 특히 유용했어요.",
      isRead: true,
    },
  ];
  ...
  const loadRagData = async (
    node: Node<{ label: string; content: string; subtitle: string }>,
  ) => {
    if (!node) return;
      ...
      <div className="sidebar-header">
        <div className="sidebar-tabs">
          ...
          >
            리뷰
            {unreadReviews > 0 && (
              <span className="badge">{unreadReviews}</span>
            )}
          </button>
          <button
            ...
            title={
              selectedNode ? "선택된 노드의 상세 정보" : "노드를 선택하세요"
            }
          >
            🔍 상세
          </button>
          ...
          <div className="review-tab">
            <div className="review-list">
              {reviews.map((review) => (
                <div
                  key={review.id}
                  className={`review-item ${!review.isRead ? "unread" : ""}`}
                >
              ...
              <>
                <div className="selected-node-info">
                  ...
                  <div className="node-summary">
                    ...
                    {selectedNode.data.subtitle && (
                      <p className="subtitle">{selectedNode.data.subtitle}</p>
                    )}
                  </div>
                </div>
                ...
                {isLoadingRag ? (
                  ...
                  <div className="rag-results">
                    ...
                    {ragData.relatedContent &&
                      ragData.relatedContent.length > 0 && (
                        <div className="rag-section">
                          <h5>관련 콘텐츠</h5>
...
const ChatHistorySidebar: React.FC<{
  ...
  return (
    <>
      <div
        className={`chat-history-sidebar ${isOpen ? "open" : "closed"} ${theme}`}
      >
        ...
        <div className="chat-history-content">
          <button className="new-chat-btn">➕ 새 대화 시작</button>
...
const AICareerCanvasInner: React.FC = () => {
  ...
  const [selectedNodeForSidebar, setSelectedNodeForSidebar] =
  ...
  useEffect(() => {
    const handleNodeExpand = (event: CustomEvent) => {
      ...
      console.log("Handling node expansion:", {
        nodeId,
      ...
      toast.show(
        `${style} 스타일로 ${newNodes.length}개 노드가 확장되었습니다!`,
        {
          type: "success",
        },
      );
    ...
    return () => {
      window.removeEventListener(
        "node-expand",
        handleNodeExpand as EventListener,
      );
    };
  }, [setNodes, setEdges, toast]);

  // 두 노드 간 최적 연결 방향 계산
  const calculateOptimalConnection = useCallback(
  ...
  const createConsistentEdge = useCallback(
    (
      ...
      if (connectionType === "handle_drag") {
        ...
        userSourceHandle &&
        userTargetHandle &&
        (connectionType === "context_add" || connectionType === "handle_add")
      ) {
  ...
  const handleAddNodeFromHandle = useCallback(
    (nodeId: string, direction: string) => {
        ...
        if (newEdge) {
          setEdges((prevEdges) => {
            const edgeExists = prevEdges.some(
              (e) => e.source === nodeId && e.target === newNodeId,
            );
            if (!edgeExists) {
    ...
    (params: Edge | Connection) => {
      ...
      if (sourceNode && targetNode) {
        ...
        console.log("✅ 핸들 드래그 연결 완료:", {
          ...
          handles: {
            source: newEdge.sourceHandle,
            target: newEdge.targetHandle,
          },
        });

        toast.success(
          `핸들 드래그: ${sourceNode.data?.label || "Node"} ${spatialDirection} ${targetNode.data?.label || "Node"} (${sourceDirection}→${targetDirection})`,
        );
      } else {
  ...
  const onEdgeClick = useCallback(
    (event: React.MouseEvent, edge: Edge) => {
      ...
      toast.info(
        "에지가 선택되었습니다. 양 끝의 조작점을 드래그하여 연결점을 변경하세요. ESC로 취소 가능",
      );
    },
    [isConnectMode, toast],
  );
  ...
  const onReconnect = useCallback(
    (oldEdge: Edge, newConnection: Connection) => {
      ...
      const { strokeColor } = getConnectionStyle(
      ...
      const sourceDirection = getDirectionName(
        newConnection.sourceHandle,
        true,
      );
      const targetDirection = getDirectionName(
        newConnection.targetHandle,
        false,
      );
      toast.success(
        `에지 연결점이 변경되었습니다: ${sourceDirection} → ${targetDirection}`,
      );
    ...
    (event: React.MouseEvent, node: any) => {
      ...
      if (isConnectMode) {
        if (!sourceNodeId) {
          ...
          toast.info(
            `첫 번째 노드 선택됨: ${node.data.label}. 두 번째 노드를 클릭하세요.`,
          );
          // 노드 하이라이트
          setNodes((nodes) =>
            nodes.map((n) => ({
          ...
          setEdges((prevEdges) => {
            ...
            if (!edgeExists) {
              ...
              toast.success(
                `연결 버튼: ${sourceNode.data.label} ${spatialDirection} ${targetNode.data.label} (${sourceDirection}→${targetDirection})`,
              );
  ...
  const handleContextAction = (action: string, nodeId: string) => {
    ...
    switch (action) {
      ...
      case "connect":
        ...
        toast.info(
          "연결 모드: 연결할 두 번째 노드를 클릭하세요. 자동으로 최적 방향이 선택됩니다. ESC키로 취소 가능",
        );
        // 첫 번째 노드 하이라이트
        setNodes((nodes) =>
          nodes.map((n) => ({
  ...
  const addStandaloneNode = () => {
    ...
    toast.success(
      "독립 노드가 생성되었습니다. 연결하려면 🔗 연결하기 버튼을 사용하세요.",
    );
  };
  ...
  const handleAiExpand = async (nodeId: string) => {
    ...
    toast.info(`"${targetNode.data.label}" 노드를 AI로 확장 중...`, {
      duration: 3000,
    });
    ...
    try {
      ...
      toast.success(
        `"${targetNode.data.label}" 노드가 ${newNodes.length}개의 하위 노드로 확장되었습니다!`,
        {
          duration: 4000,
          action: {
            label: "자동 정렬",
      ...
      toast.error(`노드 확장에 실패했습니다: ${error.message}`, {
        duration: 4000,
      });
    }
  };
  ...
  const connectNodesToCenter = (
    ...
    nodes.forEach((node) => {
      if (node.type !== "centerNode") {
        ...
        const hasConnection = newEdges.some(
          (e) =>
            e.source === node.id ||
            e.target === node.id ||
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(
          response.error?.message || "Failed to generate mindmap",
        );
      }
      ...
      toast.success(
        `${aiInput} 마인드맵이 생성되었습니다! (${response.data.metadata.processingTime}ms)`,
        {
          duration: 3000,
          action: {
            label: "저장",
      ...
      toast.error(`마인드맵 생성에 실패했습니다: ${error.message}`, {
        duration: 4000,
      });

      // Fallback to mock data for development
      if (import.meta.env.DEV) {
        toast.info("개발 모드: Mock 데이터로 폴백합니다...", {
          duration: 2000,
        });
    ...
    <div className={`ai-career-canvas ${theme}`}>
      ...
      {isEdgeEditMode && selectedEdge && (
        <div className="edge-edit-mode-banner">
          <span>
            ✏️ 에지 편집 모드: 조작점을 드래그하여 연결점을 변경하세요
          </span>
          <button
            onClick={() => {
              setIsEdgeEditMode(false);
      ...
      <div className="canvas-toolbar">
        ...
        <div className="toolbar-right">
          <button
            className="toolbar-btn add-node-btn"
              ...
              style:
                selectedEdge === edge.id
                  ? {
                      ...
                      strokeDasharray:
                        selectedEdge === edge.id ? "5,5" : undefined,
                    }
                  : edge.style,
            }))}
      ...
      {contextMenu.visible && (
        ...
        >
          <button
            onClick={() => handleContextAction("connect", contextMenu.nodeId)}
          >
            🔗 연결하기
          </button>
          <button
            onClick={() => handleContextAction("edit", contextMenu.nodeId)}
          >
            ✏️ 편집
          </button>
          <button
          ...
          <button
            onClick={() => handleContextAction("ai-expand", contextMenu.nodeId)}
          >
            🤖 AI 확장
          </button>
        </div>
