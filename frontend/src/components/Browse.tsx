...
type FilterTag = 'all' | '신입개발자' | '경력자' | 'AI전문가' | '리더십' | '멘토';
...
const Browse: React.FC<BrowseProps> = ({ onProfileSelect }) => {
  ...
  const [selectedCareerMap, setSelectedCareerMap] = useState<CareerMap | null>(null);
  const [showAdvancedSearch, setShowAdvancedSearch] = useState(false);
  const [advancedFilters, setAdvancedFilters] = useState<Partial<AdvancedSearchFilters>>({});
  ...
  const availableTags: FilterTag[] = ['all', '신입개발자', '경력자', 'AI전문가', '리더십', '멘토'];
  ...
  useEffect(() => {
    ...
    if (savedLikes) {
      try {
        ...
        console.error('Failed to parse liked profiles from localStorage:', error);
      }
    }
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem('browse_liked_profiles', JSON.stringify(Array.from(likedProfiles)));
  }, [likedProfiles]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('toggleNavigation', handleToggleNavigation);
  }, [viewMode, showNavigation]);

  // Mock fetch function for infinite scroll
  const fetchProfilesForInfiniteScroll = async (page: number, pageSize: number) => {
    const filters = {
      search: undefined,
  ...
  const loadProfiles = async (page: number = 1, pageSize: number = itemsPerPage) => {
    setLoading(true);
    try {
      console.log('🔍 Loading profiles with filters:', { searchQuery, selectedTags, sortBy, page, pageSize });
  ...
  const allProfilesData = React.useMemo(() => {
    ...
    const sourceProfiles = scrollMode === 'infinite' ? infiniteScroll.items : profiles;
    ...
    if (debouncedQuery) {
      ...
      result = result.filter(profile => {
        return (
          ...
          profile.userProfile.skills.some(skill => skill.toLowerCase().includes(query)) ||
          profile.userProfile.interests.some(interest => interest.toLowerCase().includes(query)) ||
          ...
          profiles.map((profile, index) => 
            index === profileIndex 
              ? { 
                  ...
                  stats: { 
                    ...profile.stats, 
                    totalLikes: profile.stats.totalLikes + (isCurrentlyLiked ? -1 : 1) 
                  } 
                }
              : profile
          );
          ...
          profiles.map((profile, index) => 
            index === profileIndex 
              ? { 
                  ...
                  stats: { 
                    ...profile.stats, 
                    followers: profile.stats.followers + (profile.isFollowed ? -1 : 1) 
                  }
                }
              : profile
          );
  ...
  return (
    <div className={`browse-container ${theme}`}>
        ...
        <div className="search-section">
          ...
          <div className="search-results-info">
            <span className="results-count">
              ...
              {!selectedTags.includes('all') && ` (필터: ${selectedTags.join(', ')})`}
            </span>
            {debouncedQuery && searchStats.searchFields.length > 0 && (
              <span className="search-fields">
                매칭 필드: {searchStats.searchFields.map(field => {
                  const fieldNames: { [key: string]: string } = {
                    name: '이름', role: '직책', bio: '소개', 
                    company: '회사', skills: '기술', interests: '관심분야', badges: '태그'
                  };
      ...
      {/* Profiles Grid/List */}
      {(scrollMode === 'pagination' ? loading : infiniteScroll.loading && infiniteScroll.items.length === 0) ? (
            ...
            >
              <div className="card-header">
                ...
                <div className="profile-info">
                  ...
                  <div className="work-info">
                    <span className="company">🏢 {profile.userProfile.company || '구직중'}</span>
                    <span className="location">📍 {profile.userProfile.location || '서울'}</span>
                  </div>
                  <p className="experience">{profile.userProfile.experience} 경험</p>
                </div>
                <button 
              ...
              <div className="skills-preview">
                ...
                {profile.userProfile.skills.length > 4 && (
                  <span className="more-skills">+{profile.userProfile.skills.length - 4}</span>
                )}
              </div>
              ...
              <div className="card-actions">
                ...
                >
                  {likedProfiles.has(profile.id) ? '❤️ 좋아요됨' : '🤍 좋아요'}
                </button>
                <button 
            ...
            <>
              ...
              {infiniteScroll.error && (
                <div className="infinite-scroll-error">
                  ...
                  <button onClick={() => infiniteScroll.loadMore()}>다시 시도</button>
                </div>
              )}
              ...
              <div ref={infiniteScroll.observerRef} className="infinite-scroll-trigger" />
            </>
          )}
