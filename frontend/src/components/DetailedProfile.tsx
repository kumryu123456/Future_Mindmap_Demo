...
const generateDetailedProfile = (profile: BrowseProfile): DetailedCareerProfile => {
  return {
    ...profile,
    ...
    aboutMe: {
      ...
      philosophy: '완벽한 코드보다는 사용자에게 가치를 전달하는 코드를 추구합니다.',
    ...
    skillCategories: {
      languages: profile.userProfile.skills.filter(skill => 
        ['Java', 'JavaScript', 'Python', 'TypeScript', 'SQL', 'Go', 'Kotlin'].includes(skill)
      ),
      backend: profile.userProfile.skills.filter(skill => 
        ['Spring Boot', 'Node.js', 'Express', 'Django', 'FastAPI'].includes(skill)
      ),
      database: ['MySQL', 'PostgreSQL', 'Redis', 'MongoDB'].filter(skill => 
        profile.userProfile.skills.includes(skill) || Math.random() > 0.5
      ),
      cloudDevOps: ['AWS', 'Docker', 'Jenkins', 'Kubernetes', 'CI/CD'].filter(skill => 
        profile.userProfile.skills.includes(skill) || Math.random() > 0.7
      ),
      tools: ['Git', 'Jira', 'Confluence', 'Notion', 'Figma'].filter(() => Math.random() > 0.6)
    },
      ...
      nodes: [
        {
          ...
          data: { 
            ...
            description: '스타트업에서 첫 개발 경력을 시작하며 웹 개발의 기초를 다졌습니다.',
            achievements: [
              '첫 상용 프로젝트 성공적 런칭',
              'React 기반 프론트엔드 개발',
              '팀 협업 프로세스 개선'
        ...
        },
        {
          ...
          data: { 
            ...
            company: 'IT Company',
            description: '다양한 프로젝트를 경험하며 풀스택 개발 역량을 키웠습니다.',
            achievements: [
              'B2B SaaS 플랫폼 개발 참여',
              'RESTful API 설계 및 구현',
              '성능 최적화로 로딩 속도 50% 개선'
        ...
        },
        {
          ...
          data: { 
            ...
            company: 'Tech Corp',
            description: '팀 리딩과 아키텍처 설계를 담당하며 기술 리더십을 발휘했습니다.',
            achievements: [
              '마이크로서비스 아키텍처 도입',
              '개발팀 5명 리딩 및 멘토링',
              'CI/CD 파이프라인 구축'
        ...
        },
        {
          ...
          data: { 
            ...
            period: '2024.01 - 현재',
            company: profile.userProfile.company || '현재 회사',
            description: '현재 포지션에서 혁신적인 프로젝트를 이끌며 조직의 기술 혁신을 주도하고 있습니다.',
            achievements: [
              'AI 기반 자동화 시스템 구축',
              '개발 생산성 40% 향상',
              '기술 블로그 운영 및 지식 공유'
...
const DetailedProfile: React.FC<DetailedProfileProps> = ({ profile, onBack, onMessage }) => {
  ...
  const handleSendMessage = async (recipientId: string, message: string, subject?: string) => {
    try {
      // TODO: API 호출로 메시지 전송
          ...
          <div className="info-section">
            ...
            <div className="about-content">
              ...
              {detailedProfile.aboutMe.philosophy && (
                <p><strong>개발 철학:</strong> {detailedProfile.aboutMe.philosophy}</p>
              )}
                ...
                <div>
                  ...
                  <ul style={{ marginTop: '0.5rem', paddingLeft: '1.5rem' }}>
                    {detailedProfile.aboutMe.careerGoals.map((goal, index) => (
                      <li key={index} style={{ marginBottom: '0.25rem', color: '#4b5563' }}>{goal}</li>
                    ))}
                  </ul>
                </div>
              )}
            ...
            <div className="skills-categories">
              {detailedProfile.skillCategories.languages.length > 0 && (
                <div className="skill-category">
                  ...
                  <div className="skill-items">
                    {detailedProfile.skillCategories.languages.map((skill, index) => (
                      <span key={index} className="skill-item primary">{skill}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {detailedProfile.skillCategories.backend.length > 0 && (
                <div className="skill-category">
                  ...
                  <div className="skill-items">
                    {detailedProfile.skillCategories.backend.map((skill, index) => (
              ...
              {detailedProfile.skillCategories.database.length > 0 && (
                <div className="skill-category">
                  ...
                  <div className="skill-items">
                    {detailedProfile.skillCategories.database.map((skill, index) => (
                      <span key={index} className="skill-item">{skill}</span>
                    ))}
                  </div>
                </div>
              ...
              {detailedProfile.skillCategories.cloudDevOps.length > 0 && (
                <div className="skill-category">
                  ...
                  <div className="skill-items">
                    {detailedProfile.skillCategories.cloudDevOps.map((skill, index) => (
                      <span key={index} className="skill-item">{skill}</span>
                    ))}
                  </div>
                </div>
              ...
              {detailedProfile.skillCategories.tools.length > 0 && (
                <div className="skill-category">
                  ...
                  <div className="skill-items">
                    {detailedProfile.skillCategories.tools.map((skill, index) => (
                      <span key={index} className="skill-item">{skill}</span>
                    ))}
                  </div>
                </div>
