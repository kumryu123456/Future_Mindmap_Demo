...
// 향상된 노드 컴포넌트
const EnhancedCareerStepNode: React.FC<EnhancedCareerStepNodeProps> = ({ data, selected, id }) => {
...
const InteractiveCareerFlow: React.FC<InteractiveCareerFlowProps> = ({ 
  ...
  const [viewMode, setViewMode] = useState<'default' | 'timeline' | 'skills'>('default');
  ...
  const getConnectionStyle = useCallback((sourceHandle: string | null | undefined, targetHandle: string | null | undefined) => {
  ...
  const initialEdges: Edge[] = useMemo(() => 
    careerFlow.edges.map(edge => {
      const { strokeColor, strokeWidth } = getConnectionStyle(edge.sourceHandle, edge.targetHandle);
      return {
        ...edge,
        type: 'smoothstep',
        animated: true,
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      if (!readOnly) {
        const { strokeColor, strokeWidth } = getConnectionStyle(params.sourceHandle, params.targetHandle);
        const newEdge = {
          ...params,
          type: 'smoothstep',
          animated: true,
